...
// FIX: Initialize GoogleGenAI with a named apiKey parameter, using process.env.API_KEY as per guidelines.
...
export async function getReviewForFile(file: FileData): Promise<Review | ReviewError> {
  try {
    ...
    return {
      filePath: file.path,
      error: `Failed to review file: ${errorMessage}`,
    };
  }
}
...
export async function runCodeReview(files: FileData[]): Promise<CodeReviewResult> {
    ...
    if (errors.length > 0 && errors.length === files.length) {
      throw new Error('All files failed to be reviewed. Please check your API key and network connection.');
    }
